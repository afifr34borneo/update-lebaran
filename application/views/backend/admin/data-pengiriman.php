						  				...
						  				<table class="table table-striped table-bordered table-hover dataTables">
											...
											<?php $no = 0; foreach ($row as $key => $vaData){ ?>
												<tr>
													...
													<td><?php echo $vaData['telp'] ?></td>
													<td><?php echo $vaData['no_mobil'] ?></td>
													<td align="center">
													   <a class="btn btn-effect btn-success" 
								                               	onclick="return ViewEdit(<?php echo $vaData['id_antar_kurir'] ?>);">
								                                	<i class="fa fa-edit"></i> Edit
								                               	</a>
													    <a  class="btn btn-danger"
															href="<?=site_url('Administrator/sp_kirim_paket/'.$vaData['id_antar_kurir'].'')?>">
						...
						<script type="text/javascript">
						  function ViewRelasi(){
						    $("#modal-paket").modal('show');
						      $.ajax({
						       type: "GET",
						       url: "<?php echo base_url()?>Administrator/add_kirim/",
						       cache: false,
						        beforeSend:function(){
					    ...
					    <script type="text/javascript">
						  function ViewEdit(id){
						    $("#modal-paket").modal('show');
						      $.ajax({
						       type: "GET",
						       url: "<?php echo base_url()?>Administrator/edit_kirim/edit/"+id,
						       cache: false,
						        beforeSend:function(){
						          $(".modal-body").html("<div align='center'><img  width='80' height=80'   src='<?php echo base_url()?>assets/images/logo/loading.gif' /></div>");
						        },
