<form 
	...
	?>
	    <tr>
          ...
          <td align="center" width="10%">
			...
			<?php if($vaData['status'] == '1'){ ?>
			<button type="button" id="act"
          			onclick="return picked(<?php echo $vaData['id_status'] ?>);"
          			class="btn btn-success btn-flat"><i class="fa fa-check"></i>
			</button> 	
			<?php }?>	
			<?php } ?>	
	...
	<a class="btn btn-flat btn-primary" target="blank" href="<?=base_url()?>Administrator/CetakSpPaket/">
	...
	$.ajax({
	    type: "GET",  
	    url: "<?php echo site_url('Administrator_Act/hapus_sp_paket/')?>/"+id,
	    cache: false,
	...
	$.ajax({
	    type: "GET",  
	    url: "<?php echo site_url('Administrator_Act/picked/')?>/"+id,
	    cache: false,
	    success:function(msg){
					$.gritter.add({
					    title: 'Notifikasi Sukses',
